...
CREATE TABLE IF NOT EXISTS projects (
  ...
  user_email TEXT,
  frappe_project_id TEXT NOT NULL, -- Frappe project ID (e.g., "PROJ-001")
...
-- Support both user_id and user_email approaches
...
COMMENT ON COLUMN projects.user_id IS 'Foreign key to users table (preferred method)';
COMMENT ON COLUMN projects.user_email IS 'User email (fallback if user_id not available)';
COMMENT ON COLUMN projects.frappe_project_id IS 'Frappe project ID (unique identifier from Frappe)';
