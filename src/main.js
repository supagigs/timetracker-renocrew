...
function createWindow() {
  ...
  if (process.platform === 'darwin') {
    ...
    const icnsPath = path.join(__dirname, '..', 'assets', 'icons', 'SupagigsIcon.icns');
    const pngPath = path.join(__dirname, '..', 'assets', 'icons', 'android-chrome-512x512.png');
    const icoPath = path.join(__dirname, '..', 'assets', 'icons', 'favicon.ico');
  ...
  mainWindow.loadFile(path.join(__dirname, 'renderer', 'screens', 'login.html'));
...
function createToastWindow(filePath, base64Data, screenIndex = 0, displayId = null) {
  try {
    ...
    newToastWin.loadFile(path.join(__dirname, 'renderer', 'toast.html'));
