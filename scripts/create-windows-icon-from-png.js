...
const pngPath = path.join(__dirname, '..', 'assets', 'icons', 'android-chrome-512x512.png');
const outputIcoPath = path.join(__dirname, '..', 'assets', 'icons', 'icon.ico');
const outputPng256Path = path.join(__dirname, '..', 'assets', 'icons', 'icon-256.png');

async function createWindowsIcon() {
  try {
    if (!fs.existsSync(pngPath)) {
      console.error('Error: assets/icons/android-chrome-512x512.png not found');
      process.exit(1);
    }

    console.log('Creating Windows icon from assets/icons/android-chrome-512x512.png...');
