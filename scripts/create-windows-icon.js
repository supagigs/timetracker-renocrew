...
const iconPath = path.join(__dirname, '..', 'assets', 'icons', 'SupagigsIcon.ico');
const outputPath = path.join(__dirname, '..', 'assets', 'icons', 'SupagigsIcon.ico');

async function createWindowsIcon() {
  try {
    if (!fs.existsSync(iconPath)) {
      console.error('Error: assets/icons/SupagigsIcon.ico not found');
      process.exit(1);
    }
