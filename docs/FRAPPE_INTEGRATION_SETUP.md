...
#### `src/renderer/screens/login.html`
...
3. This triggers IPC handler `auth:login` in `src/main.js`
...
- Make sure IPC handlers are registered in `src/main.js`
- Check that `src/preload.js` exposes `window.auth` API
